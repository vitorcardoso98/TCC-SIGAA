...
<body>
	<ul id="slide-out" class="side-nav fixed #00897b teal darken-1">
		<li><a href="${linkTo[IndexController].index}"><i
				...
				<tbody>
					<c:forEach items="${parametros}" var="parametros">
						<tr>
							...
							<td><a
								href="${linkTo[ParametroController].editar}?codigo=${parametros.codigo}">Modificar</a></td>
							<td><a
								href="${linkTo[ParametroController].excluirParametro}?codigo=${parametros.codigo}">Excluir</a></td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
	...
	<div class="row">
		...
		<div class="col s9">
			<a href="${linkTo[ParametroController].formAdicionaParametro}" class="waves-effect waves-light btn">Novo parâmetro</a>
		</div>
	</div>
</body>
</html>